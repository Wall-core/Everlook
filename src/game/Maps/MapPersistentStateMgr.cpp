...
void DungeonResetScheduler::ScheduleReset(bool add, time_t time, DungeonResetEvent event)
{
    ...
    if (itr == m_InstanceSaves.m_instanceSaveByInstanceId.end())
        sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "[DungeonReset] Instance %u [map %u]: ScheduleReset %u for unknown instance.", event.instanceId, event.mapId, event.type);
    else if (itr->second->GetMapId() != event.mapId)
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "[DungeonReset] Instance %u [map %u]: ScheduleReset %u for wrong instance [map %u]", event.instanceId, event.mapId, event.type, itr->second->GetMapId());

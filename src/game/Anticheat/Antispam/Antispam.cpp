...
float Antispam::Rate() const
{
    ...
    if (seconds < sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_RATE_GRACE_PERIOD))
        return 0.f;
...
void Antispam::Notify(const char *format, ...)
{
    ...
    if ((_lastNotification + sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_BLACKLIST_NOTIFY_COOLDOWN) * MINUTE * IN_MILLISECONDS) > WorldTimer::getMSTime())
        return;
    ...
    if (!!session)
    {
        ...
        ins.addUInt32(session->GetFingerprint());
    }
    else
    {
    ...
    sWorld.SendGMText(LANG_GM_ANNOUNCE_COLOR, "AntiSpam", message.str().c_str());
}
...
void Antispam::Silence(const char *format, ...)
{
    ...
    if (sWorld.getConfig(CONFIG_BOOL_AC_ANTISPAM_SILENCE))
    {
...
float Antispam::GetDistanceTraveled() const
{
    ...
    auto const movementTimeout = sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_REPETITION_MOVEMENT_TIMEOUT) * IN_MILLISECONDS;
    auto const lastMovement = WorldTimer::getMSTimeDiffToNow(player->m_movementInfo.ctime);

    if (lastMovement > movementTimeout)
        return 0.f;

    auto const anticheat = session->GetCheatData();

    if (!anticheat)
        return 0.f;

    return anticheat->DistanceTraveled();
}
...
uint32 Antispam::RepetitionScore() const
{
    ...
    if (total < sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_MIN_REPETITION_MESSAGES))
        return 0;

    if (auto const distanceScale = sWorld.getConfig(CONFIG_FLOAT_AC_ANTISPAM_REPETITION_DISTANCE_SCALE))
    {
    ...
    if (auto const timeScale = sWorld.getConfig(CONFIG_FLOAT_AC_ANTISPAM_REPETITION_TIME_SCALE))
    {
...
void Antispam::Analyze()
{
    ...
    if (GetAccountLevel(_account) > sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_MAX_LEVEL))
        return;
    ...
    if (auto const maxRate = sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_MAX_RATE))
    {
    ...
    if (auto const maxUniquePercentage = sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_MAX_UNIQUE_PERCENTAGE))
    {
        ...
        if (total > sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_MIN_UNIQUE_MESSAGES))
        {
    ...
    if (oldBlacklistCount != _blacklistCount)
    {
        ...
        if (_blacklistCount > sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_BLACKLIST_SILENCE) && sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_BLACKLIST_SILENCE) > 0)
        {
        ...
        else if (_blacklistCount > sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_BLACKLIST_NOTIFY) && sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_BLACKLIST_NOTIFY) > 0)
        {
            auto const minutes = static_cast<float>(WorldTimer::getMSTimeDiffToNow(_creationTime)) / (IN_MILLISECONDS * MINUTE);
    ...
    for (auto const &msg : messages)
    {
        ...
        for (auto i = 0u; i < _uniqueMessages.size(); ++i)
        {
            ...
            if (distance < sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_UNIQUENESS_THRESHOLD))
            {
                ++u.first;
                found = true;
                break;
    ...
    // if this is severe enough to silence them, do so
    if (score >= sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_REPETITION_SILENCE) && sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_REPETITION_SILENCE) > 0)
    {
        Silence("Messages are too repetitive.  Score: %u", score);
        return;
    }
    // otherwise, if it is severe enough to inform GMs, do so
    else if (score >= sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_REPETITION_NOTIFY) && sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_REPETITION_NOTIFY) > 0)
    {

...
void AntispamMgr::WorkerLoop()
{
    ...
    {
        auto const duration = sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_ANALYSIS_TIMER) * IN_MILLISECONDS;

        auto const start = std::chrono::high_resolution_clock::now();
        auto const startMS = WorldTimer::getMSTime();
        auto const expireMS = sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_CACHE_EXPIRATION) * IN_MILLISECONDS;

        if (sWorld.getConfig(CONFIG_BOOL_AC_ANTISPAM_ENABLED))
        {
...
void AntispamMgr::LoadFromDB()
{
    ...
    auto const normMask = sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_NORMALIZE_MASK);
    ...
    if (result)
        do
        {
            ...
            if (entry.empty())
            {
                sLog.Out(LOG_ANTICHEAT, LOG_LVL_ERROR, "Refusing to load empty antispam blacklist entry");
                continue;
            }
            ...
            for (auto const &b : _blacklist)
                if (b.first == entry)
                {
                    sLog.Out(LOG_ANTICHEAT, LOG_LVL_ERROR, "Duplicate entry \"%s\" in antispam blacklist", entry.c_str());
                    continue;
                }
    ...
    sLog.Out(LOG_ANTICHEAT, LOG_LVL_MINIMAL, ">> %u blacklist entries loaded and normalized", _blacklist.size());
    ...
    sLog.Out(LOG_ANTICHEAT, LOG_LVL_MINIMAL, ">> %u ASCII string replacements loaded", _asciiReplace.size());
    ...
    sLog.Out(LOG_ANTICHEAT, LOG_LVL_MINIMAL, ">> %u unicode character replacements loaded", _unicodeReplace.size());
}
...
void AntispamMgr::BlacklistAdd(const std::string &string_)
{
    ...
    auto const normEntry = NormalizeStringInternal(entry, sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_NORMALIZE_MASK));
...
uint32 AntispamMgr::CheckBlacklist(const std::string &string, std::string &log) const
{
    ...
    auto const normalizationMask = sWorld.getConfig(CONFIG_UINT32_AC_ANTISPAM_NORMALIZE_MASK);
...
uint32 AntispamMgr::CheckBlacklist(const std::string& string) const
{
    std::string unused;
    return CheckBlacklist(string, unused);
}
